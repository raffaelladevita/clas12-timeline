...
def processDirectory(dir, run) {
  ...
  def histlist =   (0..<6).collect{sec-> (0..<6).collect{sl ->
      def h1 = dir.getObject(String.format('/dc/DC_Time_even_%d_%d',(sec+1),(sl+1)))
...
def close() {
  ...
  (0..<6).each{ sec->
    (0..<6).each{sl->
      ...
      grtl.setTitle(name+" per sector per superlayer (for even timestamps)")
  ...
  out.writeFile('dc_' + name + '_even_sec_sl.hipo')
}
}
