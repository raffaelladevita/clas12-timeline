...
def processDirectory(dir, run) {
  ...
  def histlist =   (0..<6).collect{
    ...
    h1.setName("sec"+(it+1))
    h1.setTitle("FTOF p1a mass^2 for protons")
    h1.setTitleX("FTOF p1a mass^2 for protons (GeV^2)")
...
def close() {
  ...
  ['mean', 'sigma'].each{ name ->
    ...
    (0..<6).each{ sec->
      ...
      grtl.setTitle("FTOF p1a mass^2 for protons (" + name +")")
      grtl.setTitleY("FTOF p1a mass^2 for protons (" + name +")" + " (GeV^2)")
