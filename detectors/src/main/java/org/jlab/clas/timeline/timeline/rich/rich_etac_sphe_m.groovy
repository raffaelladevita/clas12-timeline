...
def data = new ConcurrentHashMap()
  ...
  def processDirectory(dir, run) {
    (1..2).each{module->
      ...
      def h1 = dir.getObject("/RICH/H_RICH_detac_m${module}_top3")
      int nb = h1.getDataSize(0)
      H2F h2 = h1.rebinX(nb);

      ArrayList<H1F> h2_sl = h2.getSlicesX();
      H1F h2_prox = h2_sl.get(0);
      h2_prox.setTitle("RICH Module ${module} sector ${sec}, #theta_C(meas)-#theta_C(calc) for e- (spherical photons)")
  ...
  def close() {
    ['mean', 'rms'].each{ name ->
      ...
      data.sort{it.key}.each{ttl, runs->
        ...
        grtl.setTitle("Cherenkov angle shift ${name} for e- (spherical photons), ${ttl}")
        grtl.setTitleY("#Delta #theta_C ${name} (mrad), ${ttl}")
       ...
       out.writeFile("rich_etac_sphe_m_${name}.hipo")
    }
