...
def data = new ConcurrentHashMap()
  ...
  def processDirectory(dir, run) {
    (1..2).each{module->
      ...
      def h1 = dir.getObject("/RICH/H_RICH_npbar_tile_m${module}")
  ...
  def close() {
    ['rat'].each{ name ->
      ...
      data.sort{it.key}.each{ttl, runs->
        ...
        grtl.setTitle("RICH pbar per trigger e-, ${ttl}")
        grtl.setTitleY("N(pbar)/N(e), ${ttl}")
       ...
       out.writeFile("rich_npbar_m_${name}.hipo")
    }
