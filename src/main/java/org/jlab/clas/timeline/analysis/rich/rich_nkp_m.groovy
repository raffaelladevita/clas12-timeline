...
def data = new ConcurrentHashMap()
  ...
  def processRun(dir, run) {
    (1..2).each{module->
      ...
      def h1 = dir.getObject("/RICH/H_RICH_nkp_tile_m${module}")
  ...
  def write() {
    ['rat'].each{ name ->
      ...
      data.sort{it.key}.each{ttl, runs->
        ...
        grtl.setTitle("Number of K+ per trigger e-, ${ttl}")
        grtl.setTitleY("N(K+)/N(e), ${ttl}")
       ...
       out.writeFile("rich_nkp_m_${name}.hipo")
    }
