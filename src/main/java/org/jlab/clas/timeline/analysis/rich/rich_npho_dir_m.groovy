...
def data = new ConcurrentHashMap()
  ...
  def processRun(dir, run) {
    (1..2).each{module->
      ...
      def h1 = dir.getObject("/RICH/H_RICH_npho_m${module}_top1")
      int nb = h1.getDataSize(0)
      H2F h2 = h1.rebinX(nb);

      ArrayList<H1F> h2_sl = h2.getSlicesX();
      H1F h2_prox = h2_sl.get(0);
      h2_prox.setTitle("RICH Module ${module} sector ${sec}, Number of direct photons per e-")
  ...
  def write() {
    ['mean'].each{ name ->
      ...
      data.sort{it.key}.each{ttl, runs->
        ...
        grtl.setTitle("Number of direct photons per e-, ${ttl}")
       ...
       out.writeFile("rich_npho_dir_m_${name}.hipo")
    }
